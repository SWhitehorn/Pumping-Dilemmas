...
export default (scene) => {
  ...
  const createMiddle = (sizer) => {
    ...
    return scene.rexUI.add
      ...
      .addBackground(
        scene.add
          .rexRoundRectangle(0, 0, 1, 1, { tl: 30 }, 0xdcf0fd)
              ...
              if (scene.scene.key === "Non_RegularLevel") {
                if (scene.CYK.testMembership(scene.word)) {
                  ...
                  if (scene.word.length > scene.numStates) {
                    ...
                    popUp(
                      [
                        "The word needs to be longer than " +
                          scene.numStates +
                          " letters long",
                      ],
                      scene
  ...
  const textBox = scene.add
    .rectangle(380, 460, 230 + extraWidth * 1.2, 50, colours.WHITE)
    .setStrokeStyle(3, colours.BLACK);
  scene.textEntry = scene.add
    .text(380, 460, scene.word, {
      fontSize: "40px",
  ...
  scene.hiddenInputText = scene.plugins
    .get("rexhiddeninputtextplugin")
    .add(scene.textEntry, {
      type: "text",
      enterClose: false,
      cursor: "I",
