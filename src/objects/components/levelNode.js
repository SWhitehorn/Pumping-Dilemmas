...
export default class LevelNode {
  ...
  constructor(scene, data) {
    ...
    let shadow = this.scene.add
      .circle(this.x + 5, this.y + 10, 31, colours.BLACK)
      .setAlpha(0.3);
    this.graphic = this.scene.add
      .circle(this.x, this.y, 30, colours.BLACK)
      .setInteractive();
  ...
  getControlPoint(targetNode) {
    if (this.controlPoints && this.controlPoints[targetNode.name]) {
      ...
      return Phaser.Geom.Point.Interpolate(
        this.getPosition(),
  ...
  createLookUpDict() {
    return {
      ...
      create: {
        scene: "CreateLevel",
        data: {
          inputAutomata: this.data.automata,
          words: this.data.words,
          alphabet: this.data.alphabet,
          language: this.data.language,
          deterministic: this.data.deterministic,
          loop: this.data.loop,
        },
      },
      writeWord: {
      ...
      nonRegular: {
        ...
        data: {
          ...
          numStates: this.data.numStates,
          posKey: this.data.posKey,
        },
      },
      opening: {
        scene: "OpeningScene",
