...
export default class CreateLevel extends Level {
  ...
  create({
    ...
    let automata = inputAutomata.created
      ? inputAutomata
      : structuredClone(inputAutomata);
    ...
    this.input.on("pointermove", (pointer) => {
      ...
      if (this.draw) {
        const line = new Phaser.Geom.Line(
          this.firstState.state.graphic.x,
          this.firstState.state.graphic.y,
          pointer.x,
          pointer.y
        );
  ...
  addUIElements() {
    ...
    this.nextButton = this.startZone
      .getElement("right")
      .getElement("label")
      .getElement("icon");
  ...
  moveToTests() {
    ...
    if (this.validFA()) {
      ...
      if (this.deterministic) {
        message = [
          "Not a valid DFA!",
          "Have you got a transition for each letter on every state?",
        ];
      } else {
        message = [
          "Not a valid configuration!",
          "Have you used all the states?",
        ];
      }
      popUp(message, this);
    }
