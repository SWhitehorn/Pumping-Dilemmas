...
export default class LoopLevel extends Level {
  ...
  preload() {
    ...
    this.load.image(
      "dashedLine",
      "https://raw.githubusercontent.com/SWhitehorn/Pumping-Dilemmas/main/assets/dashedLine.png"
    );
  }
  ...
  isContained(letter, slider) {
    ...
    return (
      sliderRect.x <= letterRect.x + letterRect.width / 3 &&
      sliderRect.x + sliderRect.width >= letterRect.x + letterRect.width / 1.5
    );
  }
  ...
  updateBox(extend = 1) {
    ...
    const width =
      (this.levelObjects.rightBar.getTopLeft().x - origin.x) * extend;
  ...
  drawLetters(i) {
    ...
    this.levelObjects.letters.push(
      this.add.text(this.startingX + i * 35, this.textY, this.inputWord[i], {
        fontSize: "50px",
  ...
  addElements() {
    this.textX = this.levelObjects.letters
      .at(this.numStates - 1)
      .getTopRight().x;
  ...
  drawComputedWord() {
    ...
    for (let i = 0; i < this.word.length; i++) {
      ...
      this.levelObjects.computedLetters.unshift(
        this.add.text(this.textX - i * 20, this.textY + 50, this.word[place], {
          fontSize: "20px",
          color: colours.TEXTBLACK,
          fontFamily: "Quantico",
        })
  ...
  drawSelectedWord() {
    ...
    for (let i = 0; i < this.selectedWord.length; i++) {
      this.levelObjects.computedLetters.push(
        this.add.text(
          calculateStartingX(this.selectedWord, true) + i * 18,
          this.textY + 55,
          this.selectedWord[i],
          { fontSize: "20px", color: colours.TEXTBLACK, fontFamily: "Quantico" }
        )
      );
    }
  }
  ...
  addSlidingWindow() {
    ...
    this.levelObjects.leftBar = this.add
      .rexRoundRectangle(
        leftTop.x - 20,
        leftTop.y,
        10,
    ...
    // Add right bar
    const rightTop = this.levelObjects.letters
      .at(this.numStates - 1)
      .getTopRight();
    this.levelObjects.rightBar = this.add
      .rexRoundRectangle(
        this.textX,
        rightTop.y,
        10,
  ...
  drawBox() {
    ...
    this.levelObjects.slider = this.add
