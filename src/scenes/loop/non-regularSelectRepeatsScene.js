...
export default class Non_RegularSelectRepeats extends LoopLevel {
  ...
  create({ language, word, grammar, tutorial, numStates, posKey }) {
    ...
    const rechoose = this.add
      .text(400, this.textY + 70, "Rechoose word", {
        fontSize: "20px",
      ...
      .on("pointerup", () => {
        ...
        this.scene.start("Non_RegularLevel", {
          language,
          word,
          grammar,
          numStates,
  ...
  drawSelectedWord() {
    ...
    for (let i = 0; i < this.selectedWord.length; i++) {
      this.levelObjects.computedLetters.push(
        this.add.text(
          calculateStartingX(this.selectedWord) + i * 35,
          200,
          this.selectedWord[i],
          { fontSize: "50px", color: colours.TEXTBLACK, fontFamily: "Quantico" }
        )
      );
    }
  }
  ...
  moveBars() {
    const moveLeft = () => {
      const moveRight = () => {
        const xPosRight =
          this.levelObjects.letters[rightLetter].getTopRight().x;
  ...
  testWord() {
    ...
    if (result) {
      ...
      popUp(
        [
          "Success! That word is not in the language",
          `You have proved that there cannot be an automaton with ${this.numStates} states for this language`,
        ],
        this
      );
  ...
  createUI() {
    ...
    const buttons = {
      increase: UI.getElement("bottom")
        .getElement("left")
        .getElement("label")
        .getElement("text"),
      decrease: UI.getElement("bottom")
        .getElement("right")
        .getElement("label")
        .getElement("text"),
    };
    ...
    if (this.posKey) {
      ...
      if ((this.posKey === "middle2") | (this.posKey === "middle1")) {
        ...
        if (index < this.numStates - 1) {
          ...
          let rightLetter =
            index <= this.numStates ? index - 1 : this.numStates - 1;
        ...
        if (left < this.numStates - 1 && left !== -1) {
          return {
            leftLetter: left,
            rightLetter: left + (result[0].length - 1),
          };
        }
      }
    }
