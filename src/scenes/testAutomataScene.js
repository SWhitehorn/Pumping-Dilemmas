...
export default class TestCreateLevel extends Level {
  ...
  create({
    ...
    this.speedUp = this.UI.getElement("right")
      .getElement("label")
      .getElement("text");
  ...
  *wordGenerator(words) {
  ...
  performTest() {
    ...
    this.computing = true;
    this.removeLetters();
    const icon = this.UI.getElement("left")
      .getElement("label")
      .getElement("icon");

    this.prevTest.result
      ? icon.setFillStyle(colours.GREEN)
  ...
  drawLetters(i) {
    ...
    this.levelObjects.letters.push(
      this.add.text(this.startingX + i * 35, this.textY, this.word[i], {
        fontSize: "50px",
        color: colours.BLACK,
        fontFamily: "Quantico",
      })
